...
#drukuje dane i pakuje je w słownik (nadpisywany dla kolejnych częsteczek)
        for w1 in range (40,len(table)):
            if table[w1][0]==table[1][1]:
                if len(table[w1])>=12:
                    nazwa_czasteczki=table[w1][0]  
                    atom_name=table[w1][2] 
                    ID.append(table[w1][3]) #pierwiastek
                    x.append(table[w1][12])
                    y.append(table[w1][13])
                    z.append(table[w1][14])
                    atoms.append(atom_name)
                    noa+=1
        print (nazwa_czasteczki, noa, atoms, ID, x, y, z)
